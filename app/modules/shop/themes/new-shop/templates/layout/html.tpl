...
<head>
    <title>
        {block 'title'}
            {$page.title|default:'ntc|| guess title'}
        {/block}
    </title>
    {foreach $page.libraries.css as $path => $attribure}
        <link rel="stylesheet" type="text/css" href="{$path}" {$attribure}/>
    {/foreach}
    {foreach $page.libraries.js as  $path}
